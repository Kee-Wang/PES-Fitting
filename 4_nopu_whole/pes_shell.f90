...
implicit none
  ...
  real::coeff(1:5835) ! change to number of coefficients
...
contains
  ...
  function g(xyz)
    ...
    real,dimension(1:5835)::p   ! change to number of popynomials
                               ! (size of p in bemsa.f90)
    real,dimension(1:2055)::m  ! change to number of monomials
                              ! (size of m in bemsa.f90
    real::a0
    integer::i,j,k

    a0 = 2.5  ! the same as in fitting code
