...
!Aug 13 9:13PM version

!implicit none

double precision::xx(3,6)
double precision::ralpha(12),ralpha1(12)
...
double precision,parameter::auang=0.52917721067,hatreecm=219474.63
!For unpdating config
double precision::rr1,rr2,rr3,rr4,rr5,ttheta1,ttheta2,aalpha1,aalpha2,bbeta1,bbeta2,ttau

!For ralphagrid:
double precision::dis(12),step(12),imin,imax,istep,nralpha,outcycle(4),fxx




contains
subroutine xxralpha_init()
...
!Initialize internal---------------------------------------
!PESminimum!  Ab initio minimum
r1= 1.160!1.160!1.162!   [1.155,1.165] +-0.005     eq=1.160
r2= 1.160!1.160!1.162!   [1.155,1.165] +-0.005     eq=1.160
r3= 0.959!0.959!   [0.956,0.964] +-0.005     eq=0.959
r4= 0.959!   [0.956,0.964] +-0.005     eq=0.959
r5= 2.752!                            eq=2.752
theta1= 178.112! [175.1,181.1]  +-3      eq=178.1
theta2= 105.1!105.019!  [102.1,108.1]  +-3      eq=105.1 
...
subroutine readintw(units,ralpha)
integer::units,i
double precision::ralpha(12),num
character::sym_int

read(units,*) sym_int, ralpha(:)
...
write(units,'(A32,F10.4)') splitter,num
do i=1,12
...
subroutine wrtintw(units,ralpha,num)
implicit none
double precision::ralpha(12)
integer::units,i
character::sym(12)*10
...
write(units,'(F8.1,5F7.3,7F9.3)') num,&
ralpha(1),ralpha(1),ralpha(3),ralpha(4),ralpha(5),ralpha(6),ralpha(7),ralpha(8),ralpha(9),ralpha(10),ralpha(11),ralpha(12)
end subroutine

subroutine wrtintw_heading(units)
implicit none
integer::units
write(units,'(A8,5A7,7A9)') '#Num',&
'r1','r2','r3','r4','r5','theta1','theta2','alpha1','alpha2','beta1','beta2','tau'
end subroutine
...
subroutine cyclesize(dis,step,outcycle,cout)
integer::cout,coutotal
double precision::dis(12),step(12)
double precision::outcycle(4),a

couttotal = 0
do a=outcycle(1),outcycle(2),outcycle(3)
cout = 0
do rr1 = -dis(1),dis(1),step(1)
do rr2 = -dis(1),dis(2),step(2)
do rr3 = -dis(3),dis(3),step(3)
do rr4 = -dis(4),dis(4),step(4)
do rr5 = -dis(5),dis(5),step(5)
do ttheta1 = -dis(6),dis(6),step(6)
do ttheta2 = -dis(7),dis(7),step(7)
do aalpha1 = -dis(8),dis(8),step(8)
do aalpha2 = -dis(9),dis(9),step(9)
do bbeta1 = -dis(10),dis(10),step(10)
do bbeta2 = -dis(11),dis(11),step(11)
do ttau = -dis(12),dis(12),step(12)
cout = cout+1
couttotal = couttotal+1
end do
end do
end do
end do
...
write(*,*) 'Total cycle: ',couttotal,'Inner cycle: ',cout

return

end subroutine

subroutine ralphagrid(dis,step,cout,xx,ralpha,fxx)
use pes_shell
!======================================================
!In this subrouitne, it generate a 12-d ralpha grid, convert it into xx,
!calculate all the E and then update all ralpha value (r1,r2...tau), orginal
!initialized (r1..tau) will be replaced.
!======================================================
implicit none

integer::cout
double precision,allocatable::allxx(:,:),Energy(:),allralpha(:,:)
double precision::dis(12),step(12),xx(3,6),outcycle(4),ralpha(12)
double precision::fxx
integer,dimension(1):: Min_loc

allocate(allxx(18,cout))
allocate(allralpha(12,cout))
allocate(Energy(cout))
...
do ttau = -dis(12),dis(12),step(12)
cout = cout+1
                  ralpha1(1) = ralpha(1) + rr1
                  ralpha1(2) = ralpha(2)
                  ralpha1(3) = ralpha(3) + rr3
                  ralpha1(4) = ralpha(3)
                  ralpha1(5) = ralpha(5) + rr5
                  ralpha1(6) = ralpha(6) + ttheta1
                  ralpha1(7) = ralpha(7) + ttheta2
                  ralpha1(8) = ralpha(8) + aalpha1
                  ralpha1(9) = ralpha(9) + aalpha2
                  ralpha1(10)= ralpha(10) + bbeta1
                  ralpha1(11)= ralpha(11) + bbeta2
                  ralpha1(12)= ralpha(12) + ttau
                  call intcart(ralpha1,xx)
                  do i = 1,6
                    allxx(3*i-2:3*i,cout) = xx(:,i)
                  end do
                  allralpha(:,cout)=ralpha1(:)
                  xx=xx/auang
                  Energy(cout) = f(xx)
                  xx=xx*auang
...
Min_loc = Minloc(Energy)
fxx = Minval(Energy)*hatreecm

do i = 1,6
  xx(:,i) = allxx(3*i-2:3*i,Min_loc(1))
end do


!Update configuration and used it for next cycle. The returned value is updated
!ralpha.
ralpha(:) = allralpha(:,Min_loc(1))

deallocate(allxx)
deallocate(allralpha)
deallocate(Energy)
...
subroutine rlxgrid(dis,step,nralpha,outcycle,xx,ralpha,fxx)
use pes_shell
implicit none

integer::cout,cout2,ralphanumber
double precision::dis(12),step(12),xx(3,6),outcycle(4),ralpha(12)
double precision::fxx,nralpha,a
...
open(unit=units(1),status='unknown',file=files(1))
call wrtintw_heading(units(1))
open(unit=units(2),status='unknown',file=files(2))
open(unit=units(3),status='unknown',file=files(3))

call cyclesize(dis,step,outcycle,cout)

cout2 = 0
do a=outcycle(1),outcycle(2),outcycle(3)
cout2 = cout2 + 1

ralpha((int(outcycle(4)))) = a

call ralphagrid(dis,step,cout,xx,ralpha,fxx)
call wrtintw(units(1),ralpha,fxx)
call wrtcart(units(2),xx,real(cout2))
write (units(3),*) ralpha(int(outcycle(4))),fxx
write (*,*) ralpha(int(outcycle(4))),fxx
