...
module opt
  ...
  ! method p=p-LD^{-1}L'g                         !
  !===============================================!
  subroutine optg(p,stat,file)  !This file is log file
!Stat means flag : 0,1,2
! p is just the opt_x
    double precision,dimension(:),intent(inout)::p !This is 1D coor
...
!    open(13,status='unknown',file='last.opt')
  ...
  !===========================================!
  subroutine gradient(p,grad) !input is 1D coor, output is gradient
    double precision,dimension(:),intent(in)::p
    double precision,dimension(:),intent(out)::grad
    ! ::::::::::::::::::::
    double precision,dimension(1:size(p,1))::tp !transpose?
