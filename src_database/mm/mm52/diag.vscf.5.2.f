...
C**TEMPORARY (DIMENSIONS)
     ...
     1NTOTL2(10),NTOTL3(10),LANDUM,TOLCUT,EVLCUT
      COMMON/GIVEN/LGIV
      COMMON/DEGEN/IDEGEN
...
C**TEMPORARY
C     IF(ICID.LE.0)THEN
C         WRITE(IOUT,*)'LCOUNT,ICID = ',LCOUNT,ICID
C       WRITE(IOUT,*)'BASIS'
C       DO I=1,NN
C         WRITE(IOUT,*)(ISTAT(I,J),J=1,NMODE)
C       END DO
C       WRITE(IOUT,*)'ORDER MATRIX = ',NN
C       J0=0
C       DO I=1,NN
C         WRITE(IOUT,*)
C         WRITE(IOUT,*)'COLUMN ',I
C         WRITE(IOUT,*)(XA(J0+J),J=1,I)
C         J0=J0+I
C       END DO
C     END IF
C
...
C     WRITE(IOUT,*)
C     WRITE(IOUT,*)(XA(NN-21+I),I=1,20)
C**TEMPORARY
...
C         ISTART=ISTART+1
C         IEND=IEND-1
C         IF(ISTART.GE.IEND)GO TO 15
C         DO I=1,NVAL
C           TEMP=XK(ISTART,I)
C           XK(ISTART,I)=XK(IEND,I)
C           XK(IEND,I)=TEMP
C         END DO
C         GO TO 14
15        CONTINUE
          IF(EVLCUT.NE.0)EVL=EVLCUT
        END IF
      ELSE
        DO 8 I=1,NN
C**TEMPORARY
C     WRITE(IOUT,*)(XK(J,I),J=1,I)
C     IF(NV.EQ.0.AND.ICID.EQ.1)WRITE(IOUT,*)(XK(J,I),J=1,I)
C**TEMPORARY
        DO 8 J=1,I
8       XK(I,J)=XK(J,I)
        IF(NV.LE.0)THEN
...
C**I2 IS POSITION OF 'ZERO' IN FIRST VECTOR
      ...
      NN1=ICUTX-1
      IF(IRET.GT.0.AND.NV.LT.0)THEN
        DO 33 I=1,NN1
        L=I+1
        DO 33 J=L,ICUTX
        IF(EVAL(J)-EVAL(I))33,33,44
44      A1=EVAL(I)
        EVAL(I)=EVAL(J)
        EVAL(J)=A1
        DO 22 K=1,NN
...
3       CONTINUE
      END IF
C**TEMPORARY
C     IF(NV.EQ.0.AND.ICID.EQ.0)WRITE(IOUT,*)
C    1(EVAL(J)*WAVENM,J=1,NVAL)
C**TEMPORARY
C**********************************************************
...
C**SIZE OF SINGLE UNIT
        IF(LINEAR.AND..NOT.TRIAT)THEN
          NSIZE=LINSIZ
          IF(MOD(NOAB(2),2).NE.0)NSIZE=LINSZ(KKINLP)
        END IF
      END IF
C**TEMPORARY
C     WRITE(IOUT,*)'INITIAL NSIZE = ',NSIZE
C**TEMPORARY
      NSX=NS
      IOFF=NSX-NVSYM
...
C**START VALUE OF L
        LTAU=KKINLP-1
C**KSYM=1 (COS.2TAU); KSYM=2 (SIN.2TAU)
        KSYM=KKINLP
C**NS IS SEQUENCE SYMMMETRY
C**ISYMD IS ROTATIONAL SYMMETRY
        NSX=ISYMP(ISYMD(KROT,1),NS)
        KKEL=(KROT-1)/KSTEP+1
        KKAA=KKEL/2
C**ISYMT IS TORSIONAL SYMMETRY FOR SIN(TAU/2)
C       IF(MOD(KKAA,2).NE.0)NSX=ISYMP(NSX,ISYMT)
C**ITAUSM IS TORSIONAL SYMMETRY FOR SIN(TAU)
        NSX=ISYMP(NSX,ITAUSM(KKINLP))
C**NSX IS OVERALL SYMMETRY
      END IF
C**TEMPORARY
C     WRITE(IOUT,*)'NS,NSX = ',NS,NSX
C     CALL FLUSH(IOUT)
C**TEMPORARY
12    CONTINUE
      KROTT=KROT
16    IF(KROTT.GT.4)THEN
        KROTT=KROTT-4
        GO TO 16
      END IF
      IF(.NOT.TRIAT.AND..NOT.LINEAR)THEN
        NSX=ISYMP(ISYMD(KROTT,1),NS)
        KKEL=(KROT-1)/KSTEP+1
        KKAA=KKEL/2
        IF(MOD(KKAA,2).NE.0)NSX=ISYMP(NSX,ISYMT)
      END IF
C**ZERO BLOCK
      IF((.NOT.LINEAR.OR.TRIAT).AND.NTOT(NSX).EQ.0)GO TO 133
C**TEMPORARY
C     WRITE(IOUT,*)'IMAX,NSIZE = ',IMAX,NSIZE
C**TEMPORARY
...
C**TEMPORARY
C         KROT=2*KKA
C         IF(KKA.EQ.0.OR.KKA+KKC.NE.JTHIS)KROT=KROT+1
C**TEMPORARY
          JROT=KROT
        END IF
        KKEL=(JROT-1)/KSTEP+1
        ...
        KKC=IASKC(KKEL,NSX)
C**TEMPORARY
C     WRITE(IOUT,*)'KKEL,NSX,KKA,KKC = ',KKEL,NSX,KKA,KKC
C**TEMPORARY
C**TEMPORARY
C     WRITE(IOUT,*)'I,KROT,ICYC,NSX,IMAX = ',IMAX,KKEL,ICYC,NSX,
C    1IASSIG(IMAX,KKEL,1,NSX)
C**TEMPORARY
...
C**TEMPORARY (DIMENSIONS)
      ...
      IASKA(KROT,NS)=KA
      IASKC(KROT,NS)=KC
C     WRITE(IOUT,*)'SVASS - KROT,NS,KA,KC = ',KROT,NS,KA,KC
      RETURN
      END
C*******************************************************************
C*******************************************************************
