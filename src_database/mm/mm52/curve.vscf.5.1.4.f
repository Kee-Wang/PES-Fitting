...
C****************************************************************
C****************************************************************
      SUBROUTINE FLIP(NATOM,NMODE,NBF,MBF,NVF,MVF,OMEGA,XL,MODINT,XLS,
     1MAXBAS,MEFF,XMODQ,MAXJ,ICI,IREACT)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DIMENSION NBF(NMODE),MBF(NMODE),NVF(NMODE),MVF(NMODE),
     1MODINT(NMODE)
      DIMENSION MEFF(NMODE,2),XMODQ(NMODE)
...
C**N0 IS CURRENT LOCATION - N IS NEW LOCATION
          ...
          NVFS=NVF(IREACT)
          MVFS=MVF(IREACT)
          IF(MEFF(IREACT,1).GT.IREACT)THEN
            MEFFS1=MEFF(IREACT,1)-1
          ELSE
            MEFFS1=MEFF(IREACT,1)
          END IF
          IF(MEFF(IREACT,2).GT.IREACT)THEN
            MEFFS2=MEFF(IREACT,2)-1
          ELSE
            MEFFS2=MEFF(IREACT,2)
          END IF
        ...
        NVF(N)=NVF(N0)
        MVF(N)=MVF(N0)
        IF(MEFF(N0,1).EQ.IREACT)THEN
          MEFF(N,1)=NMODE
        ELSE
          IF(MEFF(N0,1).GT.IREACT)THEN
            MEFF(N,1)=MEFF(N0,1)-1
          ELSE
            MEFF(N,1)=MEFF(N0,1)
          END IF
        END IF
        IF(MEFF(N0,2).EQ.IREACT)THEN
          MEFF(N,2)=NMODE
        ELSE
          IF(MEFF(N0,2).GT.IREACT)THEN
            MEFF(N,2)=MEFF(N0,2)-1
          ELSE
            MEFF(N,2)=MEFF(N0,2)
          END IF
        END IF
...
C****************************************************************
C****************************************************************
      SUBROUTINE TORSPT(M,H,XQ,XJ,XP,LV,INDT,NVF,NATOM,XX,XM,RR)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C*****************************
C**STILL NOT UPDATED FOR PLOTS
C*****************************
C**   GET INTEGRAL COS(L.PHI) AND SIN(L.PHI)
      LOGICAL LINEAR
      DIMENSION XX(NATOM,3,722),XM(NATOM),RR(NATOM,NATOM)
      DIMENSION H(2*LV,M,3,1),XQ(M),XJ(M),NVF(1),XP(2*LV,M)
...
501   FORMAT(8X,'TAU(RAD)',10X,'GAUSS WEIGHT (DTAU = 2*PI/M)',3X,
     1'POTENTIAL(CM-1)',/)
502   FORMAT(2X,F20.12,2X,D25.12,2X,D25.12)
C**DSTAU CONTAINS dS/dTAU IF IFLINT=1
C**DSTAU CONTAINS dS/dGAMMA IF IFLINT=2, WHERE TAU=2*GAMMA
C**TEMPORARY
      WRITE(IOUT,*)'INIT,IFLAUD,IFLINT = ',INIT,IFLAUD,IFLINT
C**TEMPORARY
      IF(INDT.EQ.0)THEN
...
C**TEMPORARY
      ...
      WRITE(IOUT,*)'INCREMENT: ',2*SP*RAD,' DEGREES'
C**DUMP NO. PTS. TO (60) IF IDUMP > 0 (NB...YLAM DUMMY)
      IF(ISCFCI.GT.0.AND.ICI.LT.0.AND.IDUMP.NE.0)THEN
        YLAM=0
C**SIN(0) MISSING FOR J=0
        WRITE(INP4)NVF(NSMODE)-1,2*LV
        WRITE(INP4)YLAM,M
        WRITE(INP4)(XQ(J),J=1,M)
      END IF
C******************************************************
      DO I=1,M
        XJ(I)=PI2/M
...
CCCC  IF(ITAU.GT.362)ITAU=ITAU-360
      ...
      DO I=1,NATOM
        DO K=1,3
          XX(I,K,1)=XX(I,K,ITAU)/DSQRT(XM(I))
        END DO
      END DO
      CALL GETPOT(VDP,NATOM,XX,RR)
      IF(IPRINT.GT.0)WRITE(IOUT,502)XQ(J),XJ(J),VDP*WAVENM
      XJS=DSQRT(XJ(J))
C**XJS IS THE WEIGHT
      DO I=1,L
        XP(I,J)=XJS
      ...
      DO LL=LV-2+1,LV-2+LMAX
        ...
        IF(LL.EQ.0)GO TO 3
        DO II=1,LL
          IF(IND.EQ.1)T=II*IFLINT
          IF(IND.EQ.2)T=0.5D0*(2*II-1)*IFLINT
          H(I,J,1,IND)=DSIN(T*XQ(J))
          IF(.NOT.LINEAR)THEN
            H(I,J,2,IND)=T*DCOS(T*XQ(J))/DSTAU(ITAU)
          ELSE
            H(I,J,2,IND)=T*DCOS(T*XQ(J))
          END IF
          H(I,J,3,IND)=-T*T*DSIN(T*XQ(J))
          I=I+1
          H(I,J,1,IND)=DCOS(T*XQ(J))
          IF(.NOT.LINEAR)THEN
            H(I,J,2,IND)=-T*DSIN(T*XQ(J))/DSTAU(ITAU)
          ELSE
            H(I,J,2,IND)=-T*DSIN(T*XQ(J))
          END IF
          H(I,J,3,IND)=-T*T*DCOS(T*XQ(J))
          I=I+1
        END DO
3       CONTINUE
        IX=1
...
55    CONTINUE
C**TEMPORARY
      WRITE(IOUT,*)'NNTAU,MMTAU = ',2*LV,M
      WRITE(IOUT,*)'COS(0)'
      WRITE(IOUT,*)(H(2,J,1,1),J=1,M)
      WRITE(IOUT,*)'COS(2)'
      WRITE(IOUT,*)(H(4,J,1,1),J=1,M)
C**TEMPORARY
      RETURN
      END
C****************************************************************
