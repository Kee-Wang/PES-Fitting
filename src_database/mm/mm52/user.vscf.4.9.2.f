...
C     DIMENSION X0(NATOM,3),OMEGA(NMODE),XL(NATOM,NMODE,3)
      DIMENSION X0(NATOM,3),OMEGA(NXMODE),XL(NATOM,NXMODE,3)
      COMMON/FILASS/IOUT
      PI=DACOS(-1.D0)
      PI2=PI/2
      PI4=PI/4
...
C**SPECIFIC TO CH4 - MODIFIED FOR HCCH
C******************************************************************
        ...
        DO J=1,NXMODE
          IF(J.LT.NXMODE)THEN
            IF(DABS(OMEGA(J)-OMEGA(J+1))*WAVENM.LT.1.D-3)THEN
              IF(DABS(OMEGA(J)-OMEGA(J-1))*WAVENM.LT.1.D-3)GO TO 100
              IF(J.LT.NXMODE-1)THEN
                IF(DABS(OMEGA(J)-OMEGA(J+2))*WAVENM.LT.1.D-3)GO TO 100
              END IF
C             CALL ROTATV(NATOM,NXMODE,XL,J,1)
C**ROTATION ANGLE ZERO
              IF(DABS(XL(2,J,2)+XL(3,J,2)).LT.1.D-5)GO TO 100
C**ROTATION ANGLE INFINITE
              IF(DABS(XL(2,J+1,2)+XL(3,J+1,2)).LT.1.D-5)GO TO 100
              TH=(XL(2,J,2)+XL(3,J,2))/
     1           (-(XL(2,J+1,2)+XL(3,J+1,2)))
              TH=DATAN(TH)
              CTH=DCOS(TH)
              STH=DSIN(TH)
              DO I=1,NATOM
