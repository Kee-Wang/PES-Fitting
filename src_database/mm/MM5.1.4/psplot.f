...
c          endif
                    ...
                    if(distpt.eq.0.) then ! Don't use this point
                      print *,'possible other contour with distpt= 0.',
     +                npt,ncc
   ...
   40       continue
            if(ifind.eq.0) then !All clear to box corner
              ...
              if(nfill.gt.maxfil) then
                ...
                print *,'nc,rcval= ',nc,rcval(nc)
                print *,ii,jj,smarr(1,1),smarr(2,1),smarr(2,2),smarr
     +          (1,2)
                do n=1,ncrv
                  do nn=1,nump(n)
...
c set up label scaling
      ...
      if (gl.ne.0. 0.and. (abs(gl).lt.0. 1.or. abs(gl).ge.1.e5)) ioffdt
     += 1
      if (ha.ne.0. 0.and. (abs(ha).lt.0. 1.or. abs(ha).ge.1.e5)) ioffdt
     += 1
      ash = 10.**(3-ifix(alog10(amax1(abs(gl),abs(ha),abs(gp)))-500.)-
     +500)
...
c  get min and max values of z for legend print
  ...
  120 format('0','MIN AND MAX CONTOURED ARRAY VALUES ARE ',2E15.5)
      ...
      if(ilabb.ne.-999.and.ileg.ne.0) then
        if (icnst .ne. 0) then
          call keksym(0.00,-.25,sizen,23HCONSTANT FIELD DETECTED, 0.,23,
     +    0)
...
C*****CURVE
      subroutine currve(x1,y1,x2,y2,x3,y3,x4,y4,contin)
      implicit real*4 (a-h,o-z)
...
c ! Add .05" for space on either side of label
   ...
   40 ipen = z(ix,iy).ne.spval .and. z(ix2,iy2).ne.spval .and. z
...
c continue line segment
   ...
   65 xx2=(fx(x,y)-xor)/xsc
      ...
      if(nsav.eq.0) then
        ...
        xmin=amin1(xmin,xx2)
        ymin=amin1(ymin,yy2)
      else if(abs(xx2-xsav(nsav)).gt.1.e-6.or. abs(yy2-ysav(nsav)).gt.1.
     +e-6) then
        nsav=nsav+1
        if(nsav.gt.ntotpt) then
          print *,'nsav too big in drlin2, program abandoned'
...
c next line modified by RT (30/7/00)
      if(ilabl.ne.1) then
c     if(ilabb.eq.-999.or.ilabl.ne.1) then
        if(nsav.eq.0) return
        ncs=0
   85   ncs=ncs+1
...
c next line modified by RT (30/7/00)
          if(ncrv.ne.ncrvs) call sldcrv(xx(1,ncrv),yy
c         if(ncrv.ne.ncrvs.and.ilabb.ne.-999) call sldcrv(xx(1,ncrv),yy
     +    (1,ncrv),npts,0.)
          npts=1
          ncrv=ncrv+1
          rcval(ncrv)=cv
   ...
   90   continue
        if(npts.ne.1) then
          nump(ncrv)=npts
c next line modified by RT (30/7/00)
          call sldcrv(xx(1,ncrv),yy(1,ncrv),npts,0.)
c         if(ilabb.ne.-999)call sldcrv(xx(1,ncrv),yy(1,ncrv),npts,0.)
        endif
        return
      endif
c-----------------------------------------------------------------------------
...
c Continue line segment
   ...
   65 xx2=(fx(x,y)-xor)/xsc
      ...
      if(nsav.eq.0) then
        ...
        xmin=amin1(xmin,xx2)
        ymin=amin1(ymin,yy2)
      else if(abs(xx2-xsav(nsav)).gt.1.e-4.or. abs(yy2-ysav(nsav)).gt.1.
     +e-4) then
        nsav=nsav+1
        if(nsav.gt.maxpt) then
          print *,'nsav too big in drlindsh, program abandoned'
...
c  factor now.
     ...
     +    (scrc,132))
        ...
        if(yarg.ne.0.) then
          scrc=' '
          write(scrc,
     1    '(''/Ypos Ypos'',f8.2,'' add def'')')yarg*conver
          cmdstr=cmdstr(1:lenstr(cmdstr,132))//' '// scrc(1:lenstr
     +    (scrc,132))
        endif
      endif
...
c  raise overbar up slightly.
      do n=1,nchar
        if(ichar(titlec(n:n)).ge.65.and.ichar(titlec(n:n)).le.90) then
          dist=size+.08
        elseif(ichar(titlec(n:n)).eq.98..or. ichar(titlec(n:n)).eq.100.
     +  or. ichar(titlec(n:n)).eq.102.or. ichar(titlec(n:n)).eq.104.or.
...
c  raise overbar up slightly.
      ...
      icap=0
      do n=1,nchar
        if(ititle.le.24.or.ititle.eq.26.or.ititle.eq.28.or. ititle.eq.
     +  30.or.ititle.eq.32.or.ititle.eq.35.or. ititle.eq.38.or.ititle.eq
     +  .49.or.ititle.eq.50) then
          dist=size+.1
          icap=1
          goto 10
        endif
...
c  Everyone else
C**SC
      open(iunit,file=fileout,status='unknown',form='formatted',
     1recl=132)
      ...
      cmdstr=
     +'%Library Creator: Kevin E. Kohler <kevin@ocean.nova.edu>'
      call filler
...
c       cmdstr='/Slw {inch setlinewidth} def'
      write(cmdstr,'(''/Slw {'',f7.4,'' mul setlinewidth} def'')')
     +        conver
      call filler
...
c  size is height of subscripted variable
      ...
      cmdstr=' '
      write(cmdstr,
     1'(''/Xpos Xpres '',f8.2,'' add def '')')xshift*conver
      call filler
      cmdstr=' '
      write(cmdstr,
     1'(''/Ypos Ypres '',F8.2,'' sub def '')')yshift*conver
      call filler
...
C  Superscript part
      ...
      cmdstr=' '
      write(cmdstr,
     1'(''/Xpos Xpres '',f8.2,'' sub def '')')xshift*conver
      call filler
      cmdstr=' '
      write(cmdstr,
     1'(''/Ypos Ypres '',F8.2,'' add def '')')yshift*conver
      call filler
