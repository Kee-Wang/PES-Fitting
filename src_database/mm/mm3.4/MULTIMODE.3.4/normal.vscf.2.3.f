...
*************************************************************************
        ...
        parameter (maxmodes=180,np=60)
        dimension xm(natom),xlat(natom,3),omega(nmode),
     &  xl(natom,nmode,3),x0(natom,3),rr(natom,natom)
        dimension coord0(maxmodes)
        dimension fijk(6*maxmodes,1),derives(maxmodes,maxmodes),
     &          potens(-1:1,-1:1)
        dimension rnorms(maxmodes,maxmodes),coord(maxmodes),
     &          freq(maxmodes),cmass(np),flag(maxmodes)
        dimension fv1(maxmodes),fv2(maxmodes)
        htr = 219474.64d0
c        bohr = 0.52917706d0
        bohr = 1.0d0
        au = 0.22937105d0
...
*************************************************************************
                derives(i,j) = (potens(1,1) - potens(-1,1) -
     &            potens(1,-1) + potens(-1,-1))/(8.0d0*delta**2)
                derives(j,i) = derives(i,j)
                else
*************************************************************************
...
*************************************************************************
                ...
                derives(i,i) = (potens(1,0) - 2*potens(0,0) +
     &            potens(-1,0))/(2.0d0*delta**2)
              endif
              enddo
              enddo
*************************************************************************
*  The derives matrix is passed to rs.  The diagonal force constants are*
...
*  the eigenvectors.                                                    *
*************************************************************************
        call rs(maxmodes,nmodes,derives,freq,1,rnorms,fv1,fv2,ierr)
