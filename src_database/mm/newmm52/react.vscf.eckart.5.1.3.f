...
C***************************************************************
      ...
      COMMON/REACTN/IDUMMY,MMTAU,INIT,INCTAU,IFLAUD,IFLINT
      COMMON/RETURN/IRET
...
C*******************************************************************
C*******************************************************************
C**INTERCHANGE X & Y SO THAT YZ IS SYMMETRY PLANE AT 'CIS'
      DO I=1,NATOM
        XSAVE=X0(I,1)
        X0(I,1)=X0(I,2)
        X0(I,2)=XSAVE
...
CC    IF(MCHECK.NE.0)CALL CHECKM(XM,X0,XX,RR,NATOM)
...
C** ??????
C       CALL GETSAY(NATOM,XX(1,1,ITAU-1),XM,XX(1,1,ITAU+1),XR,RR)
...
C**************************************
C**************************************
C**SET GEOMETRIES CORRECTLY ABOUT 'TRAN'
C**************************************
C**************************************
      IPLUS=JNIT
      IMINUS=JNIT
      DO I=1,359
        ...
        DO J=1,NATOM
          DO K=1,3
            XX(J,K,IMINUS)=XX(J,K,IPLUS)
            IF(K.EQ.JNITCD)XX(J,K,IMINUS)=-XX(J,K,IPLUS)
          END DO
        END DO
      END DO
C**GET PHASE ITAU=722 CORRECT (THIS HAS BEEN MOVED FROM
C**TAU TO 4*PI - TAU
      ICH=0
      IX=1
      DO I=2,NATOM
        IF(DABS(XX(I,1,721)).GT.DABS(XX(IX,1,721)))IX=I
      END DO
      IF(XX(IX,1,721).GE.0.AND.XX(IX,1,722).LT.0)ICH=1
      IF(XX(IX,1,721).LE.0.AND.XX(IX,1,722).GT.0)ICH=1
      IF(ICH.NE.0)THEN
        DO I=1,NATOM
          XX(I,1,722)=-XX(I,1,722)
        END DO
      END IF
      ICH=0
      IY=1
      DO I=2,NATOM
        IF(DABS(XX(I,2,721)).GT.DABS(XX(IY,2,721)))IY=I
      END DO
      IF(XX(IY,2,721).GE.0.AND.XX(IY,2,722).LT.0)ICH=1
      IF(XX(IY,2,721).LE.0.AND.XX(IY,2,722).GT.0)ICH=1
      IF(ICH.NE.0)THEN
        DO I=1,NATOM
          XX(I,2,722)=-XX(I,2,722)
        END DO
      END IF
      ICH=0
      IZ=1
      DO I=2,NATOM
        IF(DABS(XX(I,3,721)).GT.DABS(XX(IZ,3,721)))IZ=I
      END DO
      IF(XX(IZ,3,721).GE.0.AND.XX(IZ,3,722).LT.0)ICH=1
      IF(XX(IZ,3,721).LE.0.AND.XX(IZ,3,722).GT.0)ICH=1
      IF(ICH.NE.0)THEN
        DO I=1,NATOM
          XX(I,3,722)=-XX(I,3,722)
        END DO
      END IF
C*******************************************************************
C*******************************************************************
...
C**************************************
C**************************************
C**SET DERIVATIVES CORRECTLY ABOUT 'TRAN'
C**************************************
C**************************************
      IPLUS=JNIT
      IMINUS=JNIT
      DO I=1,359
        ...
        DO J=1,NATOM
          DO K=1,3
            XXP(J,K,IMINUS)=XXP(J,K,IPLUS)
            IF(K.NE.JNITCD)XXP(J,K,IMINUS)=-XXP(J,K,IPLUS)
          END DO
        END DO
      END DO
...
C**SET MINIMUM OMEGAS FOR SELECTION OF BASIS
        IF(ITAU.LT.INIT-2)THEN
          J0=0
          DO J=1,NMODE-1
            J0=J0+1
            IF(J0.EQ.IREACT)J0=J0+1
            IF(DABS(OMEGA(J,ITAU)).LT.OMEGA(J0,1))
     1      OMEGA(J0,1)=DABS(OMEGA(J,ITAU))
          END DO
        END IF
C**SET VECTORS FOR SELECTION OF BASIS
...
C**************************************
C**************************************
C**SET VECTORS CORRECTLY ABOUT 'TRAN'
C**************************************
C**************************************
...
C**TEMPORARY STORAGE UNTIL LATER
      ...
      DO I=1,359
        ...
        IF(IMINUS.LT.2)IMINUS=721
        DO L=1,NMODE-1
C**FOR THIS MODE, DETERMINE IF IN-PLANE AT 'TRAN'
          JX=1
          MINVEC=1
          DO J=1,NATOM
            DO K=1,3
              IF(DABS(XL(J,L,K,JNIT)).LT.DABS(XL(JX,L,MINVEC,JNIT)))
     1        THEN
                MINVEC=K
                JX=J
              END IF
          ...
          DO J=1,NATOM
            DO K=1,3
              XL(J,L,K,IMINUS)=XL(J,L,K,IPLUS)
              IF(K.EQ.JNITCD)XL(J,L,K,IMINUS)=-XL(J,L,K,IPLUS)
            END DO
          END DO
        END DO
      END DO
...
C**************************************
C**************************************
C**SET VECTOR DERIVATIVES CORRECTLY ABOUT 'TRAN'
C**************************************
C**************************************
      IPLUS=JNIT
      IMINUS=JNIT
      DO I=1,359
        ...
        IMINUS=IMINUS-1
        IF(IMINUS.LT.2)IMINUS=721
        DSTAU(IMINUS)=DSTAU(IPLUS)
        SPLUS(IMINUS)=SPLUS(IPLUS)
        SMINUS(IMINUS)=SMINUS(IPLUS)
        DO L=1,NMODE-1
C**FOR THIS MODE, DETERMINE IF IN-PLANE AT 'TRAN'
          JX=1
          MINVEC=1
          DO J=1,NATOM
            DO K=1,3
              IF(DABS(XL(J,L,K,JNIT)).LT.DABS(XL(J,L,MINVEC,JNIT)))
     1        THEN
                MINVEC=K
                JX=J
              END IF
          ...
          DO J=1,NATOM
            DO K=1,3
              XLP(J,L,K,IMINUS)=XLP(J,L,K,IPLUS)
              IF(K.NE.JNITCD)XLP(J,L,K,IMINUS)=-XLP(J,L,K,IPLUS)
            END DO
          END DO
        END DO
      END DO
...
C     GO TO 9876
C
C**NOW FORM 'FULL' PATH FROM TAU=0 TO TAU=4*PI
      IF(JPRINT.GT.0)WRITE(IOUT,*)'FORM FULL PATH TO 4*PI'
C**FIRST MOVE TO MORE CONVENIENT ITAU DEFINITION (ITAU=2,722 = 'CIS')
C**************************************
C**************************************
C**START REACTION PATH
C**************************************
C**************************************
      DO K=1,3
        DO I=1,NATOM
          ...
          DO ITAU=1,721
            ...
            IMINUS=IMINUS-1
            IF(IMINUS.LT.2)IMINUS=721
            XNORM(IPLUS)=XX(I,K,IMINUS)
          END DO
          DO ITAU=2,722
            XX(I,K,ITAU)=XNORM(ITAU)
          END DO
          DO ITAU=3,362
            XX(I,K,ITAU)=XX(I,K,2*ITAU-2)
          END DO
        END DO
      END DO
C**************************************
...
C**************************************
C**************************************
C**START COORDINATE DERIVATIVES
C**************************************
C**************************************
      DO K=1,3
        DO I=1,NATOM
          ...
          DO ITAU=1,721
            ...
            IMINUS=IMINUS-1
            IF(IMINUS.LT.2)IMINUS=721
            XNORM(IPLUS)=XXP(I,K,IMINUS)
          END DO
          DO ITAU=2,722
            XXP(I,K,ITAU)=XNORM(ITAU)
          END DO
          DO ITAU=3,362
            XXP(I,K,ITAU)=XXP(I,K,2*ITAU-2)
          END DO
        END DO
      END DO
C**************************************
...
C**************************************
      ...
      DO ITAU=1,721
        ...
        IMINUS=IMINUS-1
        IF(IMINUS.LT.2)IMINUS=721
        XNORM(IPLUS)=DSTAU(IMINUS)
      END DO
C**ds/dGAMMA = (ds/dTAU)(dTAU/dGAMMA)
      DO ITAU=2,722
C       DSTAU(ITAU)=XNORM(ITAU)
        DSTAU(ITAU)=XNORM(ITAU)*2
      END DO
      DO ITAU=3,362
        DSTAU(ITAU)=DSTAU(2*ITAU-2)
      END DO
C**************************************
C**************************************
C**SET JACOBIAN CORRECTLY ABOUT 'CIS'
C**************************************
C**************************************
      IPLUS=362
      IMINUS=362
...
C**************************************
      DO J=1,NMODE-1
        ...
        DO ITAU=1,721
          ...
          IMINUS=IMINUS-1
          IF(IMINUS.LT.2)IMINUS=721
          XNORM(IPLUS)=OMEGA(J,IMINUS)
        END DO
        DO ITAU=2,722
          OMEGA(J,ITAU)=XNORM(ITAU)
        END DO
        DO ITAU=3,362
          OMEGA(J,ITAU)=OMEGA(J,2*ITAU-2)
        END DO
      END DO
C**************************************
C**************************************
C**SET OMEGA CORRECTLY ABOUT 'CIS'
C**************************************
C**************************************
      IPLUS=362
      IMINUS=362
        ...
        DO J=1,NMODE-1
          DO I=1,NATOM
            ...
            DO ITAU=1,721
              ...
              IMINUS=IMINUS-1
              IF(IMINUS.LT.2)IMINUS=721
              XNORM(IPLUS)=XL(I,J,K,IMINUS)
            END DO
            DO ITAU=2,722
              XL(I,J,K,ITAU)=XNORM(ITAU)
            END DO
            DO ITAU=3,362
              XL(I,J,K,ITAU)=XL(I,J,K,2*ITAU-2)
            END DO
          END DO
        END DO
      END DO
...
C**FOR THIS MODE, DETERMINE IF IN-PLANE AT 'CIS'
          ...
          DO J=1,NATOM
            DO K=1,3
              IF(DABS(XL(J,L,K,362)).LT.DABS(XL(JX,L,MINVEC,362)))
     1        THEN
                MINVEC=K
                JX=J
              END IF
...
C**************************************
C**************************************
C**START N. COORD. VECTORS DERIVATIVES
C**************************************
C**************************************
      DO K=1,3
        DO J=1,NMODE-1
          DO I=1,NATOM
            ...
            DO ITAU=1,721
              ...
              IMINUS=IMINUS-1
              IF(IMINUS.LT.2)IMINUS=721
              XNORM(IPLUS)=XLP(I,J,K,IMINUS)
            END DO
            DO ITAU=2,722
              XLP(I,J,K,ITAU)=XNORM(ITAU)
            END DO
            DO ITAU=3,362
              XLP(I,J,K,ITAU)=XLP(I,J,K,2*ITAU-2)
            END DO
          END DO
        END DO
      END DO
...
C**FOR THIS MODE, DETERMINE IF IN-PLANE AT 'CIS'
          ...
          DO J=1,NATOM
            DO K=1,3
              IF(DABS(XL(J,L,K,362)).LT.DABS(XL(J,L,MINVEC,362)))
     1        THEN
                MINVEC=K
                JX=J
              END IF
...
C     IF(JPRINT.GT.0)THEN
        DO ITAU=1,721
          WRITE(IOUT,*)
          WRITE(IOUT,*)'**********************************'
          WRITE(IOUT,*)'JACOBIAN AT POINT ITAU = ',ITAU+1
          WRITE(IOUT,*)DSTAU(ITAU+1)
          WRITE(IOUT,*)
          WRITE(IOUT,*)'**********************************'
          WRITE(IOUT,*)'GEOMETRIES AT POINT ITAU = ',ITAU+1
          DO I=1,NATOM
            WRITE(IOUT,265)I,(XX(I,K,ITAU+1)/SQRT(XM(I)),K=1,3)
          END DO
          WRITE(IOUT,*)
          WRITE(IOUT,*)'**********************************'
          WRITE(IOUT,*)'COORDINATE DERIVATIVES AT POINT ITAU = ',ITAU+1
          DO I=1,NATOM
            WRITE(IOUT,265)I,(XXP(I,K,ITAU+1),K=1,3)
          END DO
          WRITE(IOUT,*)
          WRITE(IOUT,*)'**********************************'
          WRITE(IOUT,*)'NORMAL COORDINATES AT POINT ITAU = ',ITAU+1
          DO J=1,NMODE-1
            WRITE(IOUT,*)'MODE = ',J,' OMEGA = ',OMEGA(J,ITAU+1)*WAVENM
            DO I=1,NATOM
              WRITE(IOUT,*)(XL(I,J,K,ITAU+1),K=1,3)
            END DO
          END DO
          WRITE(IOUT,*)
          WRITE(IOUT,*)'**********************************'
          WRITE(IOUT,*)'NORMAL COORDINATE DERIVAIVES AT POINT ITAU = ',
     1    ITAU+1
          DO J=1,NMODE-1
            WRITE(IOUT,*)'MODE = ',J,' OMEGA = ',OMEGA(J,ITAU+1)*WAVENM
            DO I=1,NATOM
              WRITE(IOUT,*)(XLP(I,J,K,ITAU+1),K=1,3)
            END DO
          END DO
          WRITE(IOUT,*)'*********************************************'
          WRITE(IOUT,*)'*********************************************'
...
C     END IF
C
C**RE-EVALUATE SPLUS,SMINUS FOR 'MILLER'
      DO ITAU=2,722
C**FORM REACTION COORDINATE (S) AT ITAU+1, ITAU-1
        X=0
        DO I=1,NATOM
