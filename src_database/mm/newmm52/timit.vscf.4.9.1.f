...
C**************** Timer for SGI *******************
      EXTERNAL FDATE,DTIME
      REAL*4 DTIME,TARRAY(2),ETIME
      character*24 fdate
 ...
 1000  FORMAT(1X,'Last step ',F15.5,
     &       ' secs  ( user:',F15.5,' s and sys:',F15.5,' s)')
 1001  FORMAT(1X,'Total elapsed time', F15.5,' s.')
      RETURN
 4     TTT=DTIME(ARRAY)
      WRITE(IOUT,1001) ETIME(TARRAY)
      RETURN
 5     WRITE(IOUT,100) FDATE()
 100   FORMAT(1X,'JOB ENDED AT:',A24)
      RETURN
      END
C######################################################################
C     SUBROUTINE TIMIT(INDEX)
